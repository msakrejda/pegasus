...
public class Connection extends EventDispatcher implements IConnection, IConnectionHandler {
   ...
   private function executeNext(handler:IResultHandler, token:QueryToken, sql:String, args:Array):void {
      _currentToken = token;
      if (args) {
         doParameterizedExecute(handler, sql, args, token);
      } else {
         doExecute(handler, sql, token);
      }
   }
    ...
    * @private
    */
   private function doExecute(handler:IResultHandler, sql:String, token:QueryToken):void {
      var queryHandler:IQueryHandler = _queryHandlerFactory.createSimpleHandler(handler, token);
    ...
    * @private
    */
   private function doParameterizedExecute(handler:IResultHandler, sql:String, argValues:Array, token:QueryToken):void {
      var queryHandler:IExtendedQueryHandler = _queryHandlerFactory.createExtendedHandler(handler, token);
